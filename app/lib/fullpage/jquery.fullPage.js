	...
	$.fn.fullpage = function(options) {
		...
		function addVerticalNavigation() {
			...
			for (var i = 0; i < $('.fp-section').length; i++) {
				...
				var li = '<li><a href="#' + link + '"><span></span><i></i></a>';
		...
		function scrollPage(element, callback, isMovementUp) {
			...
			if (localStorage.getItem('__isChange__') == 'true') {
				localStorage.setItem('__isChange__', 'false')
			} else {
				if (!isMovementUp) { // 向下滚动
					let $curpage = element.prev();

					if ($curpage.hasClass('section-scroll')) {
						let $box = $curpage.find('.section-scroll-container');
						let $innerBox = $box.find('.box');
						if ($box.scrollTop() + $box.height() + 2 < $innerBox.height()) {
							return
						}
					}

				} else { // 向上滚动
					let $curpage = element.next();
					if ($curpage.hasClass('section-scroll')) {
						if ($curpage.find('.section-scroll-container').scrollTop() > 0) {
							return
						}
					}
				}
			...
			if (!isMovementUp) {
				let $box = element.find('.section-scroll-container');
				let $innerBox = $box.find('.box');
				$box.scrollTop(0);
			} else {
				let $box = element.find('.section-scroll-container');
				let $innerBox = $box.find('.box');
				$box.scrollTop($innerBox.height() - $box.height() - 2);
			}
		...
		function performMovement(v) {
			...
			if (options.css3 && options.autoScrolling && !options.scrollBar) {
				...
				if (v.element.hasClass('last-page')) { //当滚屏到最后一屏时间 
					　　　　　　
					var footer_a = $('#dowebok .last-page').prev().height(); //倒数第二屏的高度 
					　　　　　　
					var footer_h = $('#dowebok .last-page').height(); //footer的高度
					　　　　　　
		...
		$(document).on('click touchstart', '#fp-nav a', function(e) {
			...
			localStorage.setItem('__isChange__', true);
			$('.fp-section').eq(index).find('.section-scroll-container').scrollTop(0);
